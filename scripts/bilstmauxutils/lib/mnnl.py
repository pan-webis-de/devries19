...
import numpy as np
import array
from .constants import START_TAG, END_TAG
...
class FFSequencePredictor(OutputSequencePredictor):
    ...
    def prune_softmax(self, softmax_distr, word, dictionary):
        ...
        if is_in_dict(word, dictionary):
            allowed_tag_indices = [self.tag2index[tag]
                                   for tag in is_in_dict(word, dictionary) if tag in self.tag2index]
    ...
    def save_parameters(self, out_filename):
        ...
        for tag in self.index2tag.keys():
            for tag_prev in self.index2tag.keys():
                ...
                OUT.write("{} {} {}\n".format(tag_prev_name, tag_i_name,
                                              " ".join([str(x) for x in tag2tag_expression.npvalue()])))
        OUT.close()
